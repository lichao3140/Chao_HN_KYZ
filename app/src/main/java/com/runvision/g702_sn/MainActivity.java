...
import com.runvision.httpcallback.HttpQueryTicket;
...
public class MainActivity extends AppCompatActivity implements NetWorkStateReceiver.INetStatusListener, View.OnClickListener {
    ...
    private ImageView home_set, home_reg;
    ...
    public static boolean isOpenOneVsMore = true;//1:N是否对比
            ...
            switch (msg.what) {
                case Const.UPDATE_UI://更新UI
                    ...
                    if (df.format(new Date()).equals("23:30:00")) {
                        SocketDataHelper.deleteRecognitionRecord();
                        DeleteVisitorRecord();
                    }
                ...
                case Const.MSG_READ_CARD:
                    ...
                    startIdCardThread();
                    break;
                case Const.READ_CARD_INFO://内置身份证读卡器
                    ...
                    break;
                case Const.READ_CARD://收到外置身份证读卡器的信息
                    mHandler.removeMessages(Const.COMPER_FINIASH);
    ...
    private void initView() {
        ...
        home_set.setOnClickListener(view -> {
            showConfirmPsdDialog("set");
            isOpenOneVsMore = false;
        });

        home_reg = findViewById(R.id.home_reg);
        home_reg.setOnClickListener(view -> {
            showConfirmPsdDialog("reg");
            isOpenOneVsMore = false;
        });
    }
...
//            playMusic(R.raw.nolive);
    ...
    private void showAlert(int i) {
        if (i == 2 && Const.ONE_VS_MORE_TIMEOUT_NUM >= Const.ONE_VS_MORE_TIMEOUT_MAXNUM) {
            if (promptshow_xml.getVisibility() != View.VISIBLE) {
                Const.ONE_VS_MORE_TIMEOUT_NUM = 0;
                ShowPromptMessage("核验失败", 4);
            }
        ...
        if ((isOpenOneVsMore != false) || (Const.DELETETEMPLATE == false)) {
            if (AppData.getAppData().getCompareScore() <= SPUtil.getFloat(Const.KEY_ONEVSMORESCORE, Const.ONEVSMORE_SCORE) && Const.ONE_VS_MORE_TIMEOUT_NUM >= Const.ONE_VS_MORE_TIMEOUT_MAXNUM) {
                ...
                if (user.getType().equals("黑名单")) {
                    ...
                    if (user.getWordNo().equals("")) {
                        PosUtil.getWg26Status(user.getId());
                    } else {
                        try {
                            PosUtil.getWg26Status(Integer.parseInt(user.getWordNo()));
                        } catch (Exception e) {
                            LogToFile.i(TAG, "韦根发送数据异常");
                        }
                    }

                    //记录人脸识别间隔
                    SocketDataHelper.addRecognitionRecord(String.valueOf(DateTimeUtils.getLongTime()), snapImageID, user.getTemplateImageID());
                ...
                //发送到VMS
                if (socket_status.getText().toString().equals("1") && socketThread != null) {
                    SendData.sendComperMsgInfo(socketThread, true, Const.TYPE_ONEVSMORE);
                } else if (socket_status.getText().toString().equals("0")) {
                    //记录未上传比对结果到数据库
                    if (!SPUtil.getString(Const.KEY_VMSIP, "").equals("")) {
                        SocketDataHelper.addSocketRecord(String.valueOf(DateTimeUtils.getLongTime()), snapImageID, user.getTemplateImageID());
                    }
    ...
    class OneVsMoreThread extends Thread {
        ...
        public void run() {
            if (isOpenOneVsMore != false) {
                ...
                //防止连续两次识别 一定时间内不识别
                if (isOpenOneVsMore != false && user != null) {
                    if (!SocketDataHelper.isToWork(user.getId(), SPUtil.getInt(Const.KEY_FACE_RECOGNITION_TIME, Const.FACE_RECOGNITION_TIME))) {
                        Message msg = new Message();
...
//                }
    ...
    public class ReConSocketHeartBeatThread extends Thread {
        ...
        public void run() {
            super.run();
            while (Const.RECON_SOCKET) {
                try {
                    // Socket连接服务器
                    if (Const.SOCKET_STATUS && !SPUtil.getString(Const.KEY_VMSIP, "").equals("")) {
    ...
    private void showConfirmPsdDialog(String str) {
        ...
        input.setOnCompleteListener(content -> {
            if (SPUtil.getString(Const.KEY_SETTING_PASSWORD, "").equals("") && psd.equals(content)) {
                if (str.equals("reg")) {
                    Intent intent = new Intent(mContext, MyRegisterActivity.class);
    ...
    private class GetIDInfoTask extends AsyncTask<Void, Integer, Boolean> {
        ...
        protected void onPostExecute(Boolean result) {
            ...
            try {
                ...
                Log.e(TAG, "GetIDInfoTask:" + e.getMessage());
            }
        }
    }
